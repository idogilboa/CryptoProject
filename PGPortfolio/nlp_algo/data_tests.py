...
vectorizer = CountVectorizer(max_features=1500, min_df=5, max_df=0.7, ngram_range=(1, 3), stop_words=STOP_WORDS)
...
def classify_threads(train_dates_range, test_dates_range, window_length):
    train_set = query_threads_set(train_dates_range[0], test_dates_range[1], window_length, 3000)
    test_set = query_threads_set(test_dates_range[0], test_dates_range[1], window_length, 500)
...
for window in windows_length:
    print("---Windows Length: %.2f---" % (window / 3600))
    classify_threads(train_dates_range, test_dates_range, window)
